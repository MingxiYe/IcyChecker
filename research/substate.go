...
func (x SubstateAlloc) InnerStateEqual(y SubstateAlloc) (string, bool) {
	for k, xv := range x {
		// skip if k is not an inner
		if !containByList(fuzz.GetInnerValueList(), k.String()) {
			continue
		}
		yv, exist := y[k]
...
func (x *SubstateResult) EqualState(y *SubstateResult) bool {
	if x == y {
		return true
	}
